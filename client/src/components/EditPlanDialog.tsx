...
export function EditPlanDialog({ plan, open, onOpenChange, onSave }: EditPlanDialogProps) {
  const [formData, setFormData] = useState<Plan>(
    { id: 0, name: "", slug: "", price: 0, features: [], active: true }
  );
  const [featuresText, setFeaturesText] = useState("");

  // Atualizar o estado quando o prop plan muda
  useEffect(() => {
    if (plan) {
      setFormData(plan);
      ...
      setFormData({ id: 0, name: "", slug: "", price: 0, features: [], active: true });
      setFeaturesText("");
    }
  }, [plan]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {plan?.id ? "Atualize as informações do plano de assinatura" : "Configure um novo plano de assinatura"}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave}>{plan?.id ? "Salvar Alterações" : "Criar Plano"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
