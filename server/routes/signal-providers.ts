...
router.post("/", async (req, res) => {
  try {
    ...
    // === NOVA VERIFICAÇÃO: ADVANCED ANALYTICS PARA SER PROVEDOR DE SINAIS ===
    const { checkSubscription } = await import("../middleware/subscription-check");
    
    // Verificar se o usuário tem advanced analytics habilitado para ser provedor
    const mockReq = { userId: user_id } as any;
    ...
    if (!mockReq.subscription?.limits?.advancedAnalyticsEnabled) {
      return res.status(403).json({ 
        success: false,
        error: "Provisão de sinais não disponível no seu plano",
        message: "Faça upgrade para um plano que inclui Análises Avançadas para se tornar um provedor de sinais.",
        code: "SIGNAL_PROVIDER_NOT_ENABLED"
      });
    }
    
    console.log(`✅ [SIGNAL PROVIDER ALLOWED] User ${user_id}: Advanced Analytics enabled`);
...
router.post("/:id/subscribe", async (req, res) => {
  try {
    ...
    // === NOVA VERIFICAÇÃO: COPY TRADING PARA ASSINAR PROVEDOR ===
    ...
    const mockReq = { userId: subscriber_user_id } as any;
    const mockRes = {} as any;
    const mockNext = () => {};
    ...
    if (!mockReq.subscription?.limits?.copyTradingEnabled) {
      return res.status(403).json({ 
        ...
        message: "Faça upgrade para um plano que inclui Copy Trading para assinar provedores de sinais.",
        code: "COPY_TRADING_NOT_ENABLED"
      });
    }
    
    console.log(`✅ [SUBSCRIPTION ALLOWED] User ${subscriber_user_id}: Copy Trading enabled`);
