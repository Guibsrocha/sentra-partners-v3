...
export const users = mysqlTable("users", {
  ...
  language: varchar("language", { length: 10 }).default("pt-BR"), // Idioma preferido do usuário
...
export const subscriptionPlans = mysqlTable("subscription_plans", {
  ...
  economicCalendarAccess: boolean("economicCalendarAccess").default(false),
...
export const tradingAccounts = mysqlTable("trading_accounts", {
  ...
  maxFollowers: int("maxFollowers").default(10), // Máximo de contas que podem copiar esta conta
  currentFollowers: int("currentFollowers").default(0), // Número atual de contas seguindo esta conta
...
export const userSettings = mysqlTable("user_settings", {
  ...
  ntfyCopyTradeEnabled: boolean("ntfyCopyTradeEnabled").default(true),
  ntfyVpsExpiringEnabled: boolean("ntfyVpsExpiringEnabled").default(true),
  ntfySubscriptionExpiringEnabled: boolean("ntfySubscriptionExpiringEnabled").default(true),
  ntfyEaExpiringEnabled: boolean("ntfyEaExpiringEnabled").default(true),
  // Notificação de inatividade
  inactivityAlertEnabled: boolean("inactivityAlertEnabled").default(false),
  inactivityDays: int("inactivityDays").default(7), // Dias sem trades para alertar
  lastInactivityAlert: timestamp("lastInactivityAlert"), // Último alerta enviado
  dailyTime: varchar("dailyTime", { length: 5 }).default("19:00"), // Horário do resumo diário
  weeklyTime: varchar("weeklyTime", { length: 5 }).default("08:00"), // Horário do resumo semanal
  // Personalização de textos das notificações
  ntfyDailyTitle: varchar("ntfyDailyTitle", { length: 255 }).default("📊 Relatório Diário"),
  ntfyWeeklyTitle: varchar("ntfyWeeklyTitle", { length: 255 }).default("🎉 Relatório Semanal"),
  // Títulos para notificações individuais (por conta) - use {login} para o número da conta
  ntfyDailyIndividualTitle: varchar("ntfyDailyIndividualTitle", { length: 255 }).default("💰 Conta {login} - Dia"),
  ntfyWeeklyIndividualTitle: varchar("ntfyWeeklyIndividualTitle", { length: 255 }).default("📈 Conta {login} - Semana"),
  // Tipo de notificação: 'consolidated' (todas as contas somadas) ou 'individual' (uma por conta)
  ntfyNotificationType: mysqlEnum("ntfyNotificationType", ["consolidated", "individual"]).default("consolidated").notNull(),
  // Alertas de calendário econômico
  ntfyEconomicNewsEnabled: boolean("ntfyEconomicNewsEnabled").default(true),
  ntfyEconomicNewsTime: int("ntfyEconomicNewsTime").default(60), // Minutos de antecedência (padrão: 60 = 1 hora)
  ntfyEconomicNewsEmail: boolean("ntfyEconomicNewsEmail").default(true), // Enviar também por email
  // Limite de drawdown para alertas (porcentagem * 100, ex: 1000 = 10%)
  ntfyDrawdownLimit: int("ntfyDrawdownLimit").default(1000),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
/**
 * Landing Page Tracking Pixels (Admin)
 */
export const landingPagePixels = mysqlTable("landing_page_pixels", {
  id: int("id").autoincrement().primaryKey(),
  facebookPixelId: varchar("facebookPixelId", { length: 64 }),
  facebookPixelEnabled: boolean("facebookPixelEnabled").default(false),
  googleAdsId: varchar("googleAdsId", { length: 64 }),
  googleAdsEnabled: boolean("googleAdsEnabled").default(false),
  taboolaPixelId: varchar("taboolaPixelId", { length: 64 }),
  taboolaPixelEnabled: boolean("taboolaPixelEnabled").default(false),
  kwaiPixelId: varchar("kwaiPixelId", { length: 64 }),
  kwaiPixelEnabled: boolean("kwaiPixelEnabled").default(false),
  tiktokPixelId: varchar("tiktokPixelId", { length: 64 }),
  tiktokPixelEnabled: boolean("tiktokPixelEnabled").default(false),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type LandingPagePixels = typeof landingPagePixels.$inferSelect;
export type InsertLandingPagePixels = typeof landingPagePixels.$inferInsert;
...
/**
 * Economic event notifications tracking - prevents duplicate notifications
 */
export const economicEventNotifications = mysqlTable("economic_event_notifications", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
export type EconomicEventNotification = typeof economicEventNotifications.$inferSelect;
export type InsertEconomicEventNotification = typeof economicEventNotifications.$inferInsert;
...
/**
 * VPS - tracks user VPS subscriptions
 */
export const vpsSubscriptions = mysqlTable("vps_subscriptions", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  provider: varchar("provider", { length: 128 }), // Nome do provedor (ex: "AWS", "DigitalOcean")
  serverIp: varchar("serverIp", { length: 64 }),
  expirationDate: date("expirationDate").notNull(),
  isActive: boolean("isActive").default(true).notNull(),
  notificationSent7Days: boolean("notificationSent7Days").default(false),
  notificationSent5Days: boolean("notificationSent5Days").default(false),
  notificationSent3Days: boolean("notificationSent3Days").default(false),
  notificationSent1Day: boolean("notificationSent1Day").default(false),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  userIdIdx: index("userId_idx").on(table.userId),
  expirationDateIdx: index("expirationDate_idx").on(table.expirationDate),
  isActiveIdx: index("isActive_idx").on(table.isActive),
}));

export type VpsSubscription = typeof vpsSubscriptions.$inferSelect;
export type InsertVpsSubscription = typeof vpsSubscriptions.$inferInsert;

/**
 * EA Subscriptions - tracks Expert Advisor licenses
 */
export const eaSubscriptions = mysqlTable("ea_subscriptions", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  eaName: varchar("eaName", { length: 255 }).notNull(),
...
export type InsertEaSubscription = typeof eaSubscriptions.$inferInsert;

/**
 * Copy Trade Settings - tracks copy trading relationships
 */
export const copyTradeSettings = mysqlTable("copy_trade_settings", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  sourceAccountId: int("sourceAccountId").notNull(), // Conta de origem (master)
  targetAccountId: int("targetAccountId").notNull(), // Conta de destino (slave)
  isActive: boolean("isActive").default(true).notNull(),
  lotMultiplier: int("lotMultiplier").default(100), // Multiplicador * 100 (ex: 100 = 1.00x)
  maxLotSize: int("maxLotSize"), // Tamanho máximo de lote (em cents)
  enabledPairs: json("enabledPairs"), // Array de pares habilitados
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  userIdIdx: index("userId_idx").on(table.userId),
...
export type InsertCopyTradeSetting = typeof copyTradeSettings.$inferInsert;
...
export const notificationLog = mysqlTable("notification_log", {
  ...
  message: text("message"),
  metadata: json("metadata"), // Dados adicionais (accountId, tradeId, etc)
...
/**
 * Telegram Users - stores Telegram chat IDs for push notifications
 */
export const telegramUsers = mysqlTable("telegram_users", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  telegramToken: varchar("telegramToken", { length: 64 }).notNull().unique(),
  chatId: varchar("chatId", { length: 255 }).unique(),
  ...
  userIdIdx: index("userId_idx").on(table.userId),
  telegramTokenIdx: index("telegramToken_idx").on(table.telegramToken),
...
/**
 * Notification History - tracks all notifications sent via Telegram
 */
export const notificationHistory = mysqlTable("notification_history", {
  ...
  eventType: varchar("eventType", { length: 20 }),
}, (table) => ({
  userIdIdx: index("userId_idx").on(table.userId),
  typeIdx: index("type_idx").on(table.type),
  sentAtIdx: index("sentAt_idx").on(table.sentAt),
...
/**
 * Bug Reports - relatórios de bugs enviados pelos usuários
 */
export const bugReports = mysqlTable("bug_reports", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId"), // Pode ser null se usuário não estiver logado
  description: text("description").notNull(),
  page: varchar("page", { length: 255 }), // URL da página onde o bug ocorreu
  userAgent: varchar("userAgent", { length: 500 }), // Navegador/dispositivo
...
/**
 * Drawdown Alert History - rastreia alertas de drawdown enviados para deduplicação
 * Garante máximo de 2 alertas por dia por conta/usuário, espaçados 12h
 */
export const drawdownAlertHistory = mysqlTable("drawdown_alert_history", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  accountNumber: varchar("accountNumber", { length: 64 }), // null para alertas consolidados
  alertType: mysqlEnum("alertType", ["individual", "consolidated"]).notNull(),
  drawdownPercent: int("drawdownPercent").notNull(), // Armazenado como inteiro (ex: 1550 = 15.50%)
  sentAt: timestamp("sentAt").defaultNow().notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  userIdIdx: index("userId_idx").on(table.userId),
  ...
  sentAtIdx: index("sentAt_idx").on(table.sentAt),
  // Índice composto para busca rápida de alertas recentes
  userAccountTypeIdx: index("user_account_type_idx").on(table.userId, table.accountNumber, table.alertType),
}));

export type DrawdownAlertHistory = typeof drawdownAlertHistory.$inferSelect;
export type InsertDrawdownAlertHistory = typeof drawdownAlertHistory.$inferInsert;

/**
 * Copy Trading Limits - gerencia limites de followers por conta
 */
export const copyTradingLimits = mysqlTable("copy_trading_limits", {
  id: int("id").autoincrement().primaryKey(),
  masterAccountId: int("masterAccountId").notNull(), // ID da conta master
  userId: int("userId").notNull(), // ID do usuário proprietário da conta master
  maxFollowers: int("maxFollowers").default(10), // Limite máximo de followers
  currentFollowers: int("currentFollowers").default(0), // Número atual de followers
  // Limitações por plano
  maxMastersAllowed: int("maxMastersAllowed").default(1), // Máximo de contas master que pode copiar
  planLevel: varchar("planLevel", { length: 50 }).default("basic"), // Nível do plano (basic, medium, diamond, etc.)
  // Dados do plano ativo
  currentPlanId: int("currentPlanId"), // ID do plano atual do usuário
  canCopyUnlimited: boolean("canCopyUnlimited").default(false), // Pode copiar ilimitado
  isActive: boolean("isActive").default(true).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  masterAccountIdIdx: index("master_account_id_idx").on(table.masterAccountId),
  userIdIdx: index("user_id_idx").on(table.userId),
  planLevelIdx: index("plan_level_idx").on(table.planLevel),
  masterAccountUnique: index("master_account_unique_idx").on(table.masterAccountId),
}));

export type CopyTradingLimit = typeof copyTradingLimits.$inferSelect;
export type InsertCopyTradingLimit = typeof copyTradingLimits.$inferInsert;

/**
 * Copy Trading Followers - rastreia followers ativos
 */
export const copyTradingFollowers = mysqlTable("copy_trading_followers", {
  id: int("id").autoincrement().primaryKey(),
  masterAccountId: int("masterAccountId").notNull(),
  followerAccountId: int("followerAccountId").notNull(),
  masterUserId: int("masterUserId").notNull(),
  ...
  masterAccountIdIdx: index("master_account_id_idx").on(table.masterAccountId),
  followerAccountIdIdx: index("follower_account_id_idx").on(table.followerAccountId),
  masterUserIdIdx: index("master_user_id_idx").on(table.masterUserId),
  followerUserIdIdx: index("follower_user_id_idx").on(table.followerUserId),
  activeFollowsIdx: index("active_follows_idx").on(table.masterAccountId, table.isActive),
  uniqueRelationIdx: index("unique_relation_idx").on(table.masterAccountId, table.followerAccountId),
}));

export type CopyTradingFollower = typeof copyTradingFollowers.$inferSelect;
export type InsertCopyTradingFollower = typeof copyTradingFollowers.$inferInsert;

/**
 * Economic Calendar Access Logs - logs de acesso ao calendário econômico
 */
export const economicCalendarAccessLogs = mysqlTable("economic_calendar_access_logs", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  planId: int("planId").notNull(),
  hasAccess: boolean("hasAccess").notNull(),
  accessLevel: varchar("accessLevel", { length: 20 }).default("basic"),
  ...
  accessCheckIdx: index("access_check_idx").on(table.requestedAt),
}));

export type EconomicCalendarAccessLog = typeof economicCalendarAccessLogs.$inferSelect;
export type InsertEconomicCalendarAccessLog = typeof economicCalendarAccessLogs.$inferInsert;
