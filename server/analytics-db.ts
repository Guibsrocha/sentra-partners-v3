...
export async function getDrawdownHistory(accountId: number) {
  ...
  try {
    // Primeiro, tentar buscar dados de drawdown calculados
    const { getAccountDrawdown } = await import('./drawdown-calculator');
    const today = new Date();
    
    // Calcular drawdown em tempo real para garantir dados atualizados
    const currentDrawdown = await getAccountDrawdown(accountId, today, 'daily');
    const weeklyDrawdown = await getAccountDrawdown(accountId, today, 'weekly');
    const monthlyDrawdown = await getAccountDrawdown(accountId, today, 'monthly');
    
    // Buscar histórico de balance
    ...
    const balanceResult = await db.execute(balanceQuery);
    const balanceData = balanceResult.rows || [];
    
    // Combinar dados de balance com drawdown calculado
    const result = {
      balanceData: balanceData,
      drawdown: {
        daily: currentDrawdown,
        weekly: weeklyDrawdown,
        monthly: monthlyDrawdown
      }
    };
    
    console.log(`[Analytics] Drawdown data for account ${accountId}:`, {
      hasBalanceData: balanceData.length > 0,
      dailyDrawdown: currentDrawdown?.drawdownPercent || 0,
      weeklyDrawdown: weeklyDrawdown?.drawdownPercent || 0,
      monthlyDrawdown: monthlyDrawdown?.drawdownPercent || 0
    });
    ...
    return {
      balanceData: [],
      drawdown: { daily: null, weekly: null, monthly: null }
    };
  }
}
