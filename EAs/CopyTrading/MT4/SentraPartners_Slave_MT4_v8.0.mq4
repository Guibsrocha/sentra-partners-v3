    ...
    for(int i = OrdersTotal() - 1; i >= 0; i--) {
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if(OrderMagicNumber() == MagicNumber) {
                // Extrair ticket do Master do comentário (formato: "copy 123456")
                string comment = OrderComment();
                string masterTicket = comment;
                if(StringFind(comment, "copy ") == 0) {
                    masterTicket = StringSubstr(comment, 5);  // Remove "copy "
                }
    ...
    while(true) {
        ...
        for(int i = 0; i < OrdersTotal(); i++) {
            if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
                if(OrderMagicNumber() == MagicNumber) {
                    string comment = OrderComment();
                    string ticketStr = IntegerToString(masterTicket);
                    // Aceitar "copy 123456" ou "123456"
                    if(comment == ticketStr || comment == "copy " + ticketStr) {
                        exists = true;
                        break;
                    }
                }
...
string NormalizeSymbol(string masterSymbol) {
    // Testar símbolo exato primeiro
    if(TestSymbol(masterSymbol)) {
        if(EnableLogs) Print("✅ Símbolo exato: ", masterSymbol);
        return masterSymbol;
    }
    
    // Lista de sufixos comuns
    string suffixes[] = {"c", "m", ".a", ".b", "_i", "pro", "ecn", ".raw", ".lp", ".", "_"};
    
    // Tentar remover sufixos
    for(int i = 0; i < ArraySize(suffixes); i++) {
        string baseSymbol = RemoveSuffix(masterSymbol, suffixes[i]);
        if(baseSymbol != masterSymbol && TestSymbol(baseSymbol)) {
            if(EnableLogs) Print("✅ Símbolo sem sufixo: ", baseSymbol, " ← ", masterSymbol);
            return baseSymbol;
        }
    }
    ...
    for(int i = 0; i < ArraySize(suffixes); i++) {
        ...
        if(TestSymbol(testSymbol)) {
            if(EnableLogs) Print("✅ Símbolo com sufixo: ", testSymbol, " ← ", masterSymbol);
            return testSymbol;
        }
    }
    
    // Buscar em todos os símbolos do Market Watch
    string baseSymbol = StringSubstr(masterSymbol, 0, 6);  // Primeiros 6 caracteres (ex: AUDCAD)
    for(int i = 0; i < SymbolsTotal(true); i++) {
        string symbol = SymbolName(i, true);
        if(StringFind(symbol, baseSymbol) == 0) {  // Começa com baseSymbol
            if(EnableLogs) Print("✅ Símbolo encontrado no Market Watch: ", symbol, " ← ", masterSymbol);
            return symbol;
        }
    }
    
    if(EnableLogs) Print("❌ Símbolo não encontrado: ", masterSymbol);
...
// TESTAR SÍMBOLO
//====================================================
bool TestSymbol(string symbol) {
    // Resetar erro
    ResetLastError();
    
    // Tentar selecionar símbolo
    SymbolSelect(symbol, true);
    
    // Verificar se tem preço BID válido
    double bid = MarketInfo(symbol, MODE_BID);
    int error = GetLastError();
    
    return (bid > 0 && error == 0);
}
...
void OpenOrder(string symbol, int type, double lots, double sl, double tp, string comment) {
    ...
    // Adicionar "copy" no comentário
    string fullComment = "copy " + comment;
    ...
    int ticket = OrderSend(
        ...
        sl,
        tp,
        fullComment,  // "copy" + Ticket do Master
        MagicNumber,
        0,
